...
//	GlobalVariables globals = GlobalVariables.getInstance();
	...
	public KMeans(int kValue, Neuron[] neuronsTest) {
...
//		globals.kMeansClusters = new Cluster[kClusters];
		...
		distMatrix = calcDistanceMatrix(data, centroids);
		...
		int iteration = 0;
		while(!Arrays.deepEquals(distMatrix, oldDistMatrix)) {
			System.out.println("\n------------\nIteration " + iteration + "\n------------\n");
...
//					System.out.println(cluster.get(j)[i]);
	...
	private float[][] calcDistanceMatrix(Neuron[] data, float[][] centroids) {
		float[][] distanceMatrix = new float[centroids.length][];
		
		for (int j = 0; j < centroids.length; j++) {
			distanceMatrix[j] = calcDistances(data, centroids[j]);
		}
	...
	private float[] calcDistances(Neuron[] data, float[] centroid) {
		float[] distances = new float[data.length];
		
		for (int i = 0; i < data.length; i++) {
			distances[i] = DistanceCalculator.EUCLIDEAN(centroid, data[i].getAttributes());
//			distances[i] = euclidDistance(data[i].attributes, centroid);
		}
